...
public class SumVisitorImpl extends FraudoBaseVisitor<Object> {
    ...
    public Object visitSum(FraudoParser.SumContext ctx) {
        ...
        return sumAggregator.sum(CheckedField.getByValue(countTarget), fraudModel, TimeWindowResolver.resolve(ctx.time_window()));
    }

    @Override
    public Object visitSum_success(FraudoParser.Sum_successContext ctx) {
        String countTarget = TextUtil.safeGetText(ctx.STRING());
        return sumAggregator.sumSuccess(CheckedField.getByValue(countTarget), fraudModel, TimeWindowResolver.resolve(ctx.time_window()));
    }
    ...
    public Object visitSum_error(FraudoParser.Sum_errorContext ctx) {
        ...
        return sumAggregator.sumError(CheckedField.getByValue(countTarget), fraudModel, TimeWindowResolver.resolve(ctx.time_window()), errorCode);
    }
